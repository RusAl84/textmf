...
import ui.javafx.FileManager.FileWrapper;
...
public class FXText extends Application {
	...
	private GridPane makeTableView() {
		...
		Label candidatesLbl = new Label("Files");   //файлы
		GridPane.setHalignment(candidatesLbl, HPos.CENTER);
		gridPane.add(candidatesLbl, 0, 0);
		final ListView<FileWrapper> resultListView = new ListView<>(FileManager.filesProperty());
		resultListView.setPrefWidth(150);
		resultListView.setPrefHeight(150);
		resultListView.setCellFactory(new Callback<ListView<FileWrapper>, ListCell<FileWrapper>>() {
					public ListCell<FileWrapper> call(ListView<FileWrapper> param) {
		...
		TableColumn<WordWrapper, String> aliasNameCol = new TableColumn<>("Word");
		aliasNameCol.setEditable(true);
		aliasNameCol.setCellValueFactory(new PropertyValueFactory(WordWrapper.WORD));
		aliasNameCol.setPrefWidth(wordsTableView.getPrefWidth() / 3);
		TableColumn<WordWrapper, String> firstNameCol = new TableColumn<>("Weight");
		firstNameCol.setCellValueFactory(new PropertyValueFactory(WordWrapper.FIRST_VALUE));
		firstNameCol.setPrefWidth(wordsTableView.getPrefWidth() / 3);
		TableColumn<WordWrapper, String> lastNameCol = new TableColumn<>("Count");
		lastNameCol.setCellValueFactory(new PropertyValueFactory(WordWrapper.SECOND_VALUE));
		lastNameCol.setPrefWidth(wordsTableView.getPrefWidth() / 3);
		wordsTableView.getColumns().setAll(aliasNameCol, firstNameCol, lastNameCol);
		gridPane.add(wordsTableView, 2, 1);
		resultListView.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<FileWrapper>() {
					public void changed(ObservableValue<? extends FileWrapper> observable, FileWrapper oldValue, FileWrapper newValue)
								{
									if (observable != null && observable.getValue() != null) {
										...
										if (text != null) {
											...
											for (Word word : words) {
												results.add(new WordWrapper(word.toString(), String.valueOf(text.getWordWeight(word)), String.valueOf(word.getCount())));
											}
											curTextData = text;
										} else {
											// TODO: do something
		...
		wordsTableView.setRowFactory(new Callback<TableView<WordWrapper>, TableRow<WordWrapper>>() {
					public TableRow<WordWrapper> call(TableView<WordWrapper> param) {
						final TableRow<WordWrapper> cell = new TableRow<WordWrapper>() {
							...
							public void updateSelected(boolean shouldBeUpdated) {
								...
								if (shouldBeUpdated && isPressed()) {
									...
									if (curTextData != null) {
										int index = getIndex();
										if (index < curTextData.getWords().size()) {
											Word word = curTextData.getWords().get(index);
											for (String form : word.getWordForms())
												results.add(new WordWrapper(form, "", ""));
										}
										tableView.setItems(results);
										TableColumn<WordWrapper, String> wordCol = new TableColumn<>("Word");
										wordCol.setEditable(false);
										wordCol.setCellValueFactory(new PropertyValueFactory(WordWrapper.WORD));
										wordCol.setPrefWidth(tableView.getPrefWidth());
